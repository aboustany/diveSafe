...
public class MemberController {
  ...
  public static String registerMember(String email, String password, String name, String emergencyContact, int nrDays,
    ...
    Member member = diveSafe.addMember(email, password, name, emergencyContact, nrDays, guideRequired, hotelRequired);
  ...
  public static List<String> getMemberEmails() {
    return diveSafe.getMembers().stream().map(member -> member.getEmail()).toList();
  }

  public static List<String> getAssignedMemberEmails() {
    return diveSafe.getMembers().stream().filter(member -> member.hasAssignment() == true)
        .map(member -> member.getEmail()).toList();
  }
   ...
   * @return placeHolder
   */
  private static TOMember wrapMember(Member member) {
    TOMember placeHolder = new TOMember(member.getEmail(), member.getName(), member.getEmergencyContact(),
            member.getNumDays(), member.isGuideRequired(), member.isHotelRequired());
   ...
   * @return equipmentList a list of Transfer Objects for Equipment
   */
  public static List<TOMember> getMembers() {
    List<TOMember> memberList = new ArrayList<>();

    for (var member : diveSafe.getMembers()) {
      var newTOAssignment = wrapMember(member);
      memberList.add(newTOAssignment);
    }
